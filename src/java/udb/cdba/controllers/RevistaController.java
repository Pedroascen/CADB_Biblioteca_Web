...
import udb.cdba.model.RevistaModel;

/**
 *
 * @author Jazmine
 */
@WebServlet(name = "RevistaController", urlPatterns = {"/revista"})
public class RevistaController extends HttpServlet {
    ...
    protected void processRequest(HttpServletRequest request, HttpServletResponse response)
        ...
        try (PrintWriter out = response.getWriter()) {
            ...
            switch (accion) {
                ...
                case "nuevo":
                    request.getRequestDispatcher("/revista/add.jsp").forward(request, response);
                    break;
                case "insertar":
    ...
    private void insertar(HttpServletRequest request, HttpServletResponse response) {
        ...
        try {
            if (titulo != "" && ufisica != "" && cejemp != "" && editorial != "" && ISSN != "" && idioma != "" && tamano != "" && periodicidad != "") {
                RevistaModel rm = new RevistaModel();
                if (rm.insert_update(codMaterial, titulo, ufisica, cejemp, editorial, ISSN, idioma, tamano, periodicidad, fechapublicacion)) {
                    request.getRequestDispatcher("/revista/list.jsp").forward(request, response);
                } else {
                    request.getRequestDispatcher("/revista/add.jsp").forward(request, response);
                }
            }
